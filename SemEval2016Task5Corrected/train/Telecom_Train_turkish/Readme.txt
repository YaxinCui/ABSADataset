...
Please follow the steps below to fill sample data content to xml file.
	...
	4- Run command below:
		# python tweetFetcher.py "ACCESS_TOKEN" "ACCESS_TOKEN_SECRET" "CONSUMER_KEY" "CONSUMER_KEY_SECRET"

	5- That's all. The unmasked file is ready to use in current directory. (telecom_train_turkish_final_unmasked.xml)

Note: Python script file(tweetFetcher.py) and data file(telecom_train_turkish_final.xml) file should be in the same directory. Python 2.7 is required.
